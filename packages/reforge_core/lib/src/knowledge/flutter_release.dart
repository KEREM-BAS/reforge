...
/// Apple toolchain requirements enforced by a Flutter release.
@immutable
final class DarwinRequirements {
  const DarwinRequirements({required this.xcode, required this.cocoapods});

  /// Below [VersionFloor.error], iOS builds stop and `flutter doctor` reports
  /// an error; below [VersionFloor.warn], doctor warns.
  final VersionFloor xcode;

  /// Below [VersionFloor.error], the tool skips `pod install` and stops;
  /// below [VersionFloor.warn], it warns that pods handling may fail.
  final VersionFloor cocoapods;
}
...
final class FlutterRelease {
  ...
  KnowledgeSource get xcodeRequirementsSource =>
      sourceFor('packages/flutter_tools/lib/src/macos/xcode.dart');

  /// The source of [DarwinRequirements.cocoapods].
  KnowledgeSource get cocoapodsRequirementsSource =>
      sourceFor('packages/flutter_tools/lib/src/macos/cocoapods.dart');
