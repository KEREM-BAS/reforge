...
final class RubyLexer {
  RubyLexer(this.source);
...
final class RubyParser {
  RubyParser(this._tokens);
...
/// A tokenized and parsed Ruby file (Podfile, podspec).
final class RubyDocument {
  RubyDocument._(this.statements, this.comments, this.diagnostics);

  factory RubyDocument.parse(String source) {
    final lexer = RubyLexer(source)..tokenize();
    final parser = RubyParser(lexer.tokens);
    final statements = parser.parseStatements(topLevel: true);
    return RubyDocument._(statements, lexer.comments,
        List.unmodifiable([...lexer.diagnostics, ...parser.diagnostics]));
  }
  ...
  /// Every statement, depth-first, including those inside blocks.
  Iterable<RubyStatement> get allStatements sync* {
    for (final statement in statements) {
      yield statement;
      for (final block in statement.blocks) {
